...
const GreenCheckIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#28a745" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>;
...
const SortIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line></svg>;
...
const MemberDetailsPage = ({ selectedMembers = [], onBack, onContinue }) => {
        ...
        <div className="details-page-container">
            <div className="details-main-content">
                ...
                <footer className="details-footer">
                    <button className="details-continue-btn" onClick={onContinue}>Continue ›</button>
                </footer>
...
// --- PAGE 3: CITY SELECTION PAGE ---
const CitySelectionPB = ({ onBack, onContinue }) => {
    const [selectedCityPB, setSelectedCityPB] = useState(null);
    const [searchQueryPB, setSearchQueryPB] = useState('');

    const citiesPB = ['Delhi', 'Bengaluru', 'Pune', 'Hyderabad', 'Mumbai', 'Thane', 'Gurgaon', 'Chennai', 'Ghaziabad', 'Ernakulam'];
    const filteredCitiesPB = citiesPB.filter(city => city.toLowerCase().includes(searchQueryPB.toLowerCase()));
            ...
            <div className="details-main-content">
                ...
                <div className="pb-main-content">
                    <div className="pb-search-container">
                        ...
                        />
                        {searchQueryPB && ( <span className="pb-search-clear" onClick={() => setSearchQueryPB('')}>×</span> )}
                    </div>
                        ...
                        <div className="pb-city-tags">
                            {filteredCitiesPB.map((city) => (
                                <div
                                    ...
                                    onClick={() => setSelectedCityPB(city)}
                                >
                                    {city}
                                </div>
                            ))}
                 ...
                 <footer className="details-footer">
                    <button className="details-continue-btn" disabled={!selectedCityPB} onClick={onContinue}>Continue ›</button>
                </footer>
            </div>
        </div>
    );
...
// --- PAGE 4: PLANS LISTING PAGE ---
const allPlans = [
    { id: 1, insurer: 'Niva Bupa', logo: 'https://www.policybazaar.com/images/insurer-logo/niva-bupa.png', hospitals: 10200, features: ['Single private AC room', '₹1.25 lakh Renewal Bonus', 'Unlimited Restoration of cover'], coverage: 500000, price: 524, discount: 5 },
    { id: 2, insurer: 'HDFC ERGO', logo: 'https://www.policybazaar.com/images/insurer-logo/HDFC-ERGO-Health-Insurance.png', hospitals: 13000, features: ['Any room upgrade', 'Restore up to ₹5 lakh', 'No medical check-up needed'], coverage: 500000, price: 610, discount: 0 },
    { id: 3, insurer: 'Aditya Birla', logo: 'https://www.policybazaar.com/images/insurer-logo/aditya-birla.png', hospitals: 11000, features: ['Single private room', 'Up to 100% Health Returns', 'Annual Health check-up'], coverage: 500000, price: 580, discount: 10 },
    { id: 4, insurer: 'Star Health', logo: 'https://www.policybazaar.com/images/insurer-logo/star-health.png', hospitals: 14000, features: ['No Pre-policy Medical check-up', 'Cover for maternity expenses', 'Automatic Restoration of Cover'], coverage: 500000, price: 650, discount: 0 },
    { id: 5, insurer: 'ICICI Lombard', logo: 'https://www.policybazaar.com/images/insurer-logo/icici-lombard.png', hospitals: 7500, features: ['Room rent up to ₹5,000/day', 'Reset Benefit up to 100%', 'Wellness Program'], coverage: 500000, price: 540, discount: 5 },
    { id: 6, insurer: 'Care Health', logo: 'https://www.policybazaar.com/images/insurer-logo/care-health-insurance.png', hospitals: 22900, features: ['Single private AC room', 'Unlimited automatic recharge', 'Annual health checkup for all'], coverage: 500000, price: 595, discount: 0 },
];

const PlansListingPage = ({ onBack }) => {
    const [plans, setPlans] = useState([]);
    const [visibleCount, setVisibleCount] = useState(4);
    const [sortBy, setSortBy] = useState('premium-asc');

    useEffect(() => {
        let sortedPlans = [...allPlans];
        switch (sortBy) {
            case 'premium-asc':
            ...
            case 'premium-desc':
                ...
                break;
            case 'insurer-asc':
                sortedPlans.sort((a, b) => a.insurer.localeCompare(b.insurer));
                break;
            default:
                break;
        }
    ...
    return (
        <div className="plans-listing-page">
            <header className="plans-header">
                <button onClick={onBack} className="back-button"><BackArrowIcon /></button>
                <h1>Available Health Insurance Plans</h1>
            </header>

            <main className="plans-main">
                <aside className="plans-filters">
                    <h3>Sort By</h3>
                    <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                        <option value="premium-asc">Premium: Low to High</option>
                        <option value="premium-desc">Premium: High to Low</option>
                        <option value="insurer-asc">Insurer Name</option>
                    </select>
                     <h3>Filters</h3>
                    <div className="filter-group">
                        <h4>Insurers</h4>
                        {['Niva Bupa', 'HDFC ERGO', 'Aditya Birla', 'Star Health', 'ICICI Lombard', 'Care Health'].map(insurer => (
                            <label key={insurer}><input type="checkbox"/> {insurer}</label>
                        ))}
                    </div>
                </aside>
                    ...
                    {plans.slice(0, visibleCount).map(plan => (
                        <div key={plan.id} className="listing-plan-card">
                            <div className="listing-plan-header">
                                <img src={plan.logo} alt={`${plan.insurer} logo`} />
                                <div>
                                    <h3>{plan.insurer}</h3>
                                    <p>{plan.hospitals.toLocaleString('en-IN')} Cashless hospitals</p>
                                </div>
                            </div>
                            <div className="listing-plan-body">
                                <ul className="listing-plan-features">
                                    {plan.features.map(feature => <li key={feature}><GreenCheckIcon/> {feature}</li>)}
                                </ul>
                                <div className="listing-plan-pricing">
                                    <p className="coverage">Cover <span className="amount">₹{plan.coverage.toLocaleString('en-IN', {minimumFractionDigits: 0, maximumFractionDigits:0, style: 'currency', currency: 'INR'}).replace('₹', '₹ ')}</span></p>
                                    <p className="monthly-price">Starting from <span>₹{plan.price}/month</span></p>
                                    <p className="annual-price">₹{(plan.price * 12 * (1 - plan.discount / 100)).toLocaleString('en-IN')} annually</p>
                                     {plan.discount > 0 && <p className="discount-tag">{plan.discount}% Online Discount!</p>}
                                    <button className="listing-check-premium-btn">Check Premium</button>
                                </div>
                            </div>
                        </div>
                    ))}
                    {visibleCount < plans.length && 
                        <button className="show-more-plans-btn" onClick={() => setVisibleCount(plans.length)}>Show more plans</button>
                    }
                </section>
            </main>
...
function App() {
    const [page, setPage] = useState('selection'); // 'selection', 'details', 'city', or 'plans'
    ...
    const renderPage = () => {
        switch (page) {
            case 'selection':
                return <SelectionPage onContinue={handleGoToDetails} />;
            case 'details':
                return <MemberDetailsPage selectedMembers={selectedMembers} onBack={handleGoBack} onContinue={handleGoToCity} />;
            case 'city':
                return <CitySelectionPB onBack={handleGoBack} onContinue={handleGoToPlans} />;
            case 'plans':
                return <PlansListingPage onBack={handleGoBack} />;
            default:
                return <SelectionPage onContinue={handleGoToDetails} />;
        }
    };
        ...
        <>
            <style>{`
                ...
                * { box-sizing: border-box; }
                ...
                .page-container { font-family: 'Poppins', sans-serif; max-width: 1200px; margin: 2rem auto; background-color: var(--white); box-shadow: var(--shadow-md); border-radius: 16px; overflow: hidden; }
                ...
                .details-continue-btn:disabled { background-image: none; background-color: #ccc; cursor: not-allowed; box-shadow: none; }
                ...
                .pb-main-content { display: flex; flex-direction: column; width: 100%; }
                .pb-main-heading { font-size: 2rem; font-weight: 700; color: #2c3e50; margin-bottom: 2rem; text-align: center; }
                .pb-search-container { position: relative; margin-bottom: 2rem; }
                .pb-search-input { width: 100%; padding: 1rem; font-size: 1rem; border-radius: 8px; border: 1px solid #ced4da; }
                .pb-search-clear { position: absolute; right: 15px; top: 50%; transform: translateY(-50%); cursor: pointer; font-size: 1.5rem; color: #888; }
                .pb-popular-cities { margin-bottom: 2rem; }
                .pb-popular-heading { font-size: 1.1rem; font-weight: 600; margin-bottom: 1rem; color: var(--text-dark); }
                .pb-city-tags { display: flex; flex-wrap: wrap; gap: 0.75rem; }
                .pb-city-tag { padding: 0.6rem 1.2rem; border: 1px solid var(--border-color); border-radius: 20px; cursor: pointer; transition: all 0.2s ease; }
                .pb-city-tag.pb-selected, .pb-city-tag:hover { background-color: var(--primary-green); color: white; border-color: var(--primary-green); }
                .pb-tooltip { display: flex; align-items: center; background-color: #eaf6ff; border-radius: 8px; padding: 1rem; gap: 1rem; }
                .pb-tooltip-icon { background-color: #3498db; color: white; border-radius: 50%; width: 24px; height: 24px; display: grid; place-items: center; font-weight: bold; flex-shrink: 0; }
                .pb-tooltip-text { margin: 0; font-size: 0.9rem; color: var(--text-light); }

                /* --- Page 4: Plans Listing Page Styles --- */
                .plans-listing-page { font-family: 'Segoe UI', 'Montserrat', sans-serif; background-color: #f0f4f8; }
                .plans-header { background-color: var(--white); padding: 1rem 2rem; box-shadow: var(--shadow-sm); display: flex; align-items: center; gap: 1.5rem; }
                .plans-header h1 { font-size: 1.5rem; font-weight: 600; color: var(--text-dark); margin: 0; }
                .plans-main { display: flex; max-width: 1400px; margin: 2rem auto; gap: 2rem; padding: 0 2rem; }
                .plans-filters { flex: 0 0 280px; background: var(--white); border-radius: 12px; padding: 1.5rem; box-shadow: var(--shadow-sm); align-self: flex-start; }
                .plans-filters h3 { font-size: 1.2rem; margin: 0 0 1rem; }
                .plans-filters select, .plans-filters .filter-group { margin-bottom: 1.5rem; }
                .plans-filters select { width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 8px; }
                .filter-group h4 { font-size: 1rem; margin-bottom: 0.75rem; color: var(--text-dark); }
                .filter-group label { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem; font-size: 0.95rem; }
                .plans-list-container { flex: 1; }
                .why-pb-banner { background: linear-gradient(to right, #e0f7fa, #d1eaf0); border: 1px solid #b2dfdb; color: var(--text-dark); padding: 1rem 1.5rem; border-radius: 12px; margin-bottom: 2rem; text-align: center; font-size: 1.1rem; }
                .listing-plan-card { background-color: var(--white); border-radius: 16px; box-shadow: var(--shadow-md); margin-bottom: 2rem; overflow: hidden; transition: transform 0.2s; }
                .listing-plan-card:hover { transform: translateY(-5px); }
                .listing-plan-header { display: flex; align-items: center; gap: 1rem; padding: 1.5rem; border-bottom: 1px solid var(--border-color); }
                .listing-plan-header img { height: 40px; }
                .listing-plan-header h3 { margin: 0; font-size: 1.2rem; }
                .listing-plan-header p { margin: 0; font-size: 0.9rem; color: var(--text-light); }
                .listing-plan-body { display: grid; grid-template-columns: 2fr 1fr; padding: 1.5rem; }
                .listing-plan-features { list-style: none; padding: 0; margin: 0; }
                .listing-plan-features li { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem; }
                .listing-plan-pricing { text-align: right; border-left: 1px solid var(--border-color); padding-left: 1.5rem; }
                .listing-plan-pricing .coverage { font-size: 1rem; color: var(--text-light); }
                .listing-plan-pricing .coverage .amount { font-weight: 700; color: var(--text-dark); }
                .listing-plan-pricing .monthly-price { font-size: 1.1rem; margin: 0.5rem 0; }
                .listing-plan-pricing .monthly-price span { font-weight: 700; font-size: 1.5rem; color: var(--text-dark); }
                .listing-plan-pricing .annual-price { color: var(--text-light); font-size: 0.9rem; margin-bottom: 0.5rem; }
                .listing-plan-pricing .discount-tag { font-size: 0.8rem; color: #28a745; background-color: #e9f7ec; padding: 0.2rem 0.5rem; border-radius: 5px; display: inline-block; margin-bottom: 1rem; }
                .listing-check-premium-btn { background-image: linear-gradient(to right, var(--primary-orange) 0%, var(--primary-orange-dark) 51%, var(--primary-orange) 100%); background-size: 200% auto; color: white; border: none; border-radius: 8px; padding: 0.8rem 1.5rem; font-size: 1rem; font-weight: 600; cursor: pointer; width: 100%; transition: 0.5s; }
                .listing-check-premium-btn:hover { background-position: right center; }
                .show-more-plans-btn { display: block; width: fit-content; margin: 0 auto 2rem; padding: 0.8rem 2.5rem; border: 2px solid var(--primary-orange); background: transparent; color: var(--primary-orange); font-size: 1rem; font-weight: 600; border-radius: 8px; cursor: pointer; transition: all 0.3s ease; }
                .show-more-plans-btn:hover { background: var(--primary-orange); color: var(--white); }
                .plans-footer { text-align: center; padding: 2rem; font-size: 0.8rem; color: var(--text-light); }

                /* --- Responsive Styles --- */
                @media (max-width: 992px) {
                    .plans-main { flex-direction: column; }
                    .plans-filters { align-self: stretch; }
                }
                @media (max-width: 768px) {
                    .page-container { margin: 1rem; }
                    .details-body { flex-direction: column-reverse; gap: 2rem; }
                    .listing-plan-body { grid-template-columns: 1fr; gap: 1.5rem; }
                    .listing-plan-pricing { text-align: left; border-left: none; padding-left: 0; border-top: 1px solid var(--border-color); padding-top: 1.5rem; }
                }
